...
""" main code using multithreading to read from a video and run detection on the same, no scoket used"""
...
stop_threads = False
frame_counter = 0
data_appended = False  # Flag to track data append status
...
def detect_and_display():
    global current_frame, stop_threads, frame_counter, ct, data_appended
    while not stop_threads:
        if keyboard.is_pressed('esc'):
            stop_threads = True
            break
        ...
        if local_frame is not None:
            ...
            if frame_counter % 10 == 0 and not data_appended:
                a, unflattened_lst = unflatten(output)
                ct = count_first_items(unflattened_lst, ct)
                append_to_excel(excel_file_path, ct)
                data_appended = True  # Mark that data has been appended for this interval

            frame_counter += 1

            if frame_counter % 10 == 1:  # Reset right after append operation
                data_appended = False  # Ready for the next interval
                ct = {0: 0, 1: 0, 2: 0, 3: 0}  # Reset counts after appending
...
excel_file_path = "../streamlit_site/count_result.xlsx"
